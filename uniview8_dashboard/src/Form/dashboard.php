...
$sum = db_query("select COUNT(airpay_memberid) as membert,ROUND(SUM(".$pgname."_amount),2) as thisamount1 from ".$table." where ".$pgname."_statusDesc = 'success' and DATE_FORMAT(paidtime, '%Y-%m') = :amt;" ,array(':amt' => $p));
...
//drupal_set_message($tamt);
setlocale(LC_MONETARY,'en_IN');
$thisamt = money_format("%n",$tamt);

$sum1 = db_query("select COUNT(airpay_memberid) as memberp,ROUND(SUM(".$pgname."_amount),2) as thisamount2 from ".$table." where ".$pgname."_statusDesc = 'success' and DATE_FORMAT(paidtime, '%Y-%m') = :amt ;" ,array(':amt' => $t));
...
//drupal_set_message(money_format("%n",$tamt2));
...
$counts2 = db_query("select ROUND(SUM(airpay_amount),2) as users from ".$table." where DATE_FORMAT(paidtime,'%Y-%m') = :date AND ".$pgname."_statusDesc = 'success' group by ".$pgname."_memberid;",array(':date' => $date));
...
//$result = db_query("select memberid, membername, SUM(amount1), airpayamt, ROUND(SUM(amount1) - IFNULL((airpayamt),0),2) as balance , DATE_FORMAT(billdate,'%Y-%m')  from bill_details a left join (select airpay_memberid,SUM(airpay_amount) airpayamt from ".$table." where airpay_status = 's' and DATE_FORMAT(paidtime,'%Y-%m') = :bmonth group by airpay_memberid ) b on a.memberid = b.".$pgname."_memberid and amount1 >= 0 where particular1 = :TOTAL and DATE_FORMAT(billdate,'%Y-%m') = :amonth group by amount1, memberid, membername,airpayamt, DATE_FORMAT(billdate,'%Y-%m') order by balance desc limit 10", array (':TOTAL' => "TOTAL BILL AMOUNT", ':bmonth' => $dummy,':amonth' => $dummy));

$result = db_query("select memberid, membername, SUM(amount1), airpayamt, ROUND(SUM(amount1) - IFNULL((airpayamt),0),2) as balance , DATE_FORMAT(billdate,'%Y-%m')  from bill_details a left join (select airpay_memberid,SUM(airpay_amount) airpayamt from ".$table." where airpay_status = 's' and DATE_FORMAT(paidtime,'%Y-%m') = :bmonth group by airpay_memberid ) b on a.memberid = b.".$pgname."_memberid and amount1 >= 0 where particular1 != :TOTAL and particular1 != :OPEN and DATE_FORMAT(billdate,'%Y-%m') = :amonth group by memberid, membername,airpayamt, DATE_FORMAT(billdate,'%Y-%m') order by balance desc limit 10", array (':TOTAL' => "TOTAL BILL AMOUNT", ':bmonth' => $d,':amonth' => $d,':OPEN' => 'OPENING BAL'));
...
//Defaulter all time
//$result1 = db_query("select memberid, membername, SUM(amount1), airpayamt, ROUND(IFNULL(SUM(amount1),0) - IFNULL((airpayamt),0),2) as balance from bill_details a left join (select airpay_memberid, SUM(airpay_amount) as airpayamt from  ".$table." where airpay_status = 's' group by airpay_memberid ) b on a.memberid = b.".$pgname."_memberid where amount1 > 0 and   particular1 != :TOTAL and particular1 != :OPEN group by memberid, membername,airpayamt order by balance desc limit 10", array (':TOTAL' => "TOTAL BILL AMOUNT",':OPEN'=>'OPENING BAL'));
...
$result2 = db_select('bill_details','a');
	  ...
	  $result2->leftjoin($table,'b',array('airpay_memberid','airpay_status'));
          $result2->where('SUM(airpay_amount)','amount1','-');
