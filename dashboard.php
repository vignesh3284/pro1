...
$m = money_format('₹',$tmparray["airpay_amount"]);
//$v = money_format('₹',$thisamt["airpay_amount"]);
//$p = date('Y-m', strtotime('first month of current year'));
//$g =date_create("2019-01");

$p = date('Y-m', strtotime('0 month'));
$t = date('Y-m', strtotime('-1 month'));
...
$sum = db_query("select ROUND(SUM(".$pgname."_amount),2) as thisamount1 from ".$table." where ".$pgname."_status = 'S' and DATE_FORMAT(paidtime, '%Y-%m') = :amt ;" ,array(':amt' => $p));


foreach ($sum as $records) {
 $the  = $records->thisamount1;
$thisamt =$m.$the;
}


$sum1 = db_query("select ROUND(SUM(".$pgname."_amount),2) as thisamount2 from ".$table." where ".$pgname."_status = 'S' and DATE_FORMAT(paidtime, '%Y-%m') = :amt ;" ,array(':amt' => $t));


foreach ($sum1 as $records1) {

$this2  = $records1->thisamount2;
$thisamt2 =$m.$this2;
}
...
//$result = db_query("select memberid, membername, amount1, SUM(airpay_amount), (amount1 - SUM(airpay_amount)) as balance , DATE_FORMAT(billdate,'%Y%m')  from bill_details a left join uniview8_airpay_response b on a.memberid = b.airpay_memberid where particular1 = :TOTAL and b.airpay_status = 's' and DATE_FORMAT(billdate,'%Y%m') = :amonth and DATE_FORMAT(paidtime,'%Y%m') = :bmonth group by amount1, memberid, membername, DATE_FORMAT(billdate,'%Y%m') order by balance desc limit 10", array (":TOTAL" => "TOTAL BILL AMOUNT", ":bmonth" => '201904',":amonth" => '201904'));
$mon = $_GET[$today];
//$yr = $_GET['year'];
//$year = $_GET['ye'];
//$month = $_GET['mo'];
...
$d = date("Ym", strtotime('0 month'));

$c = date("Ym", strtotime('0 month'));
...
$result = db_query("select memberid, membername, amount1, SUM(".$pgname."_amount), ROUND(amount1 - SUM(".$pgname."_amount),2) as balance , DATE_FORMAT(billdate,'%Y-%m')  from bill_details a left join ".$table." b on a.memberid = b.".$pgname."_memberid where particular1 = :TOTAL and b.".$pgname."_status = 's' and DATE_FORMAT(billdate,'%Y-%m') = :amonth and DATE_FORMAT(paidtime,'%Y-%m') = :bmonth and amount1 >= 0 group by amount1, memberid, membername, DATE_FORMAT(billdate,'%Y-%m') order by balance desc limit 10", array (':TOTAL' => "TOTAL BILL AMOUNT", ':bmonth' => $d,':amonth' => $d));
...
$result2 = db_select('bill_details','a');
	  ...
	  $result2->leftjoin($table,'b',array('airpay_memberid','airpay_status'));
          $result2->condition('SUM(airpay_amount)','amount1','-');
          $result2->condition('a.memberid','b.airpay_memberid','=');
          $result2->condition('b.airpay_status','s','=');
...
$result3 = db_query("select particular1 ,ROUND(SUM(amount1),2) as totalamount from bill_details where particular1 !=:OPEN and particular1 != :TOTAL and particular1 !='' and DATE_FORMAT(billdate,'%Y-%m') = :amount group by particular1 order by totalamount DESC;",array(':TOTAL' => "TOTAL BILL AMOUNT",':OPEN' => "OPENING BAL",':amount'=> $c));
...
//drupal_set_message($y);   
      ...
      $table_data1 .= '<tr><td>' . $row->memberid .'</td><td>'. $row->membername   .'</td><td class="ritd">' . $m.$row->balance .'</td></tr>';   
...
foreach ($result as $row) {
      ...
      $table_data .= '<tr><td>' . $row->memberid .'</td><td>'. $row->membername   .'</td><td class="ritd">' . $m.$row->balance .'</td></tr>'; 
...
//drupal_set_message($y);   
      ...
      $table_data2 .= '<tr><td>' . $row->memberid .'</td><td>'. $row->membername   .'</td><td class="ritd">' . $m.$row->balance .'</td></tr>';   
...
//drupal_set_message($y);   
      ...
      $table_data3 .= '<tr><td>' . $row->particular1 .'</td><td class="ritd">' . $m.$row->totalamount .'</td></tr>';   
...
$select->condition('a.memberid','b.airpay_memberid','=');
$select->condition('particular1','TOTAL BILL AMOUNT','=');
$select->condition('b.airpay_status','s','=');
...
function dashboard_graph($height1,$height2,$height3,$height4,$height5,$height6,$height7,$height8,$height9,$height10,$height11,$height12,
                        $pay_u1,$pay_u2,$pay_u3,$pay_u4,$pay_u5,$pay_u6,$pay_u7,$pay_u8,$pay_u9,$pay_u10,$pay_u11,$pay_u12,$total_members,$thisamt,$thisamt2) {
...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en">
	...
	<body>
                        ...
                        <p class="previousmonth">Previous month</p>

                       <p class="thismonth1">'. $thisamt .'</p> 

                       <p class="previousmonth1">'. $thisamt2 .'</p> 
		...
		<li class="bar nr_5 purple"><div class="top"></div><div class="bottom"></div><span>'.$pay_u5.'</span></li>		
		<li class="bar nr_6 purple"><div class="top"></div><div class="bottom"></div><span>'.$pay_u6.'</span></li>
		<li class="bar nr_7 purple"><div class="top"></div><div class="bottom"></div><span>'.$pay_u7.'</span></li>
		<li class="bar nr_8 purple"><div class="top"></div><div class="bottom"></div><span>'.$pay_u8.'</span></li>
		<li class="bar nr_9 purple"><div class="top"></div><div class="bottom"></div><span>'.$pay_u9.'</span></li>
		<li class="bar nr_10 purple"><div class="top"></div><div class="bottom"></div><span>'.$pay_u10.'</span></li>
		<li class="bar nr_11 purple"><div class="top"></div><div class="bottom"></div><span>'.$pay_u11.'</span></li>
		<li class="bar nr_12 purple"><div class="top"></div><div class="bottom"></div><span>'.$pay_u12.'</span></li>
